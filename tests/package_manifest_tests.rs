...
fn packaging_patches_the_pinned_native_shader_before_building() {
    ...
    assert!(bash.contains("vulkan-repro-v4:[0-9a-f]{64}"));
    assert!(powershell.contains("vulkan-repro-v4:[0-9a-f]{64}"));
    assert!(powershell.contains("-cnotmatch"));
    assert!(bash.contains("verify-vulkan-shader-constants.py"));
    assert!(powershell.contains("verify-vulkan-shader-constants.py"));
...
fn patched_package_verification_rejects_every_invalid_native_patch_identity_shape() {
    for identity in [
        ...
        format!(
            "cargo=release;native_patch=llama-cpp-sys-2-0.1.151:vulkan-repro-v4:{}",
            "0".repeat(63)
        ),
        format!(
            "cargo=release;native_patch=llama-cpp-sys-2-0.1.151:vulkan-repro-v4:{}",
